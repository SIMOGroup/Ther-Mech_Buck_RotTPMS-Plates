function [K_th] = cal_Thermal_Stiffness_Matrices_2D_6dof(IGA,Plate)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Calculate thermal stiffness matrices with Quasi-3D six-variable plate theory %%%
    ...
    if abs(uKnot(ni)-uKnot(ni+1)) > tol && abs(vKnot(nj)-vKnot(nj+1)) > tol  % Check if the current element has nonzero area in the parametric domain
        ...
        for iGauss = 1: nGauss  % Loop over the integration points
            for jGauss = 1: nGauss
                ...
                Bgx0 = zeros(3,ndof*nn);  % d(u_0, v_0, w_0)/dx
                Bgx0(1,1:ndof:ndof*nn) = dNdxy(:,1)';
                Bgx0(2,2:ndof:ndof*nn) = dNdxy(:,1)';
                Bgx0(3,3:ndof:ndof*nn) = dNdxy(:,1)';
                
                Bgx1 = zeros(3,ndof*nn);  % d(-w_{0,x}, -w_{0,y}, 0)/dx
                Bgx1(1,3:ndof:ndof*nn) = -dN2dxy(:,1)';
                Bgx1(2,3:ndof:ndof*nn) = -dN2dxy(:,3)';
                
                Bgx2 = zeros(3,ndof*nn);  % d(\beta_{x}, \beta_{y}, 0)/dx
                Bgx2(1,4:ndof:ndof*nn) = dNdxy(:,1)';
                Bgx2(2,5:ndof:ndof*nn) = dNdxy(:,1)';
                
                Bgx3 = zeros(3,ndof*nn);  % d(0, 0, \beta_{z})/dx
                Bgx3(3,6:ndof:ndof*nn) = dNdxy(:,1)';

                Bgy0 = zeros(3,ndof*nn);  % d(u_0, v_0, w_0)/dy
                Bgy0(1,1:ndof:ndof*nn) = dNdxy(:,2)';
                Bgy0(2,2:ndof:ndof*nn) = dNdxy(:,2)';
                Bgy0(3,3:ndof:ndof*nn) = dNdxy(:,2)';
                
                Bgy1 = zeros(3,ndof*nn);  % d(-w_{0,x}, -w_{0,y}, 0)/dy
                Bgy1(1,3:ndof:ndof*nn) = -dN2dxy(:,3)';
                Bgy1(2,3:ndof:ndof*nn) = -dN2dxy(:,2)';
                
                Bgy2 = zeros(3,ndof*nn);  % d(\beta_{x}, \beta_{y}, 0)/dy
                Bgy2(1,4:ndof:ndof*nn) = dNdxy(:,2)';
                Bgy2(2,5:ndof:ndof*nn) = dNdxy(:,2)';
                
                Bgy3 = zeros(3,ndof*nn);  % d(0, 0, \beta_{z})/dy
                Bgy3(3,6:ndof:ndof*nn) = dNdxy(:,2)';
                
                Bgx{1} = Bgx0; Bgx{2} = Bgx1; Bgx{3} = Bgx2; Bgx{4} = Bgx3;
                Bgy{1} = Bgy0; Bgy{2} = Bgy1; Bgy{3} = Bgy2; Bgy{4} = Bgy3;
                k_th = 0;
                for i = 1:4
                    for j = 1:4
