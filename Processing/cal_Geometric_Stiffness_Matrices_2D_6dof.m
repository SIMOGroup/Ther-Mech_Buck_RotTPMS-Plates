function [Kg] = cal_Geometric_Stiffness_Matrices_2D_6dof(IGA,Plate)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Calculate geometric stiffness matrices with Quasi-3D six-variable plate theory %%%
    ...
    if abs(uKnot(ni)-uKnot(ni+1)) > tol && abs(vKnot(nj)-vKnot(nj+1)) > tol  % Check if the current element has nonzero area in the parametric domain
        ...
        for iGauss = 1: nGauss  % Loop over the integration points
            for jGauss = 1: nGauss
                ...
                Bg(1,6:ndof:ndof*nn) = gz*dNdxy(:,1)';
                Bg(2,6:ndof:ndof*nn) = gz*dNdxy(:,2)';
